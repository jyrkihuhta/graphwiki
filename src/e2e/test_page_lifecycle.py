...
class TestPageCreation:
    def test_create_page_via_missing_page(self, page: Page, base_url: str):
        """Visiting a nonexistent page redirects to the editor."""
        page.goto(f"{base_url}/page/NewTestPage")
        expect(page).to_have_url(f"{base_url}/page/NewTestPage/edit")
        expect(page.locator("#content")).to_be_visible()

    def test_create_and_save_page(self, page: Page, base_url: str):
        """Create a page via the editor and verify it renders."""
        page.goto(f"{base_url}/page/HelloWorld/edit")
        page.locator("#content").fill("# Hello World\n\nThis is a test page.")
        page.locator("button[type='submit']").click()
        page.wait_for_url(f"{base_url}/page/HelloWorld*")
    ...
    def test_page_appears_in_list(self, page: Page, base_url: str, create_page):
        ...
        create_page("ListTest", "# List Test")
        page.goto(base_url)
        expect(page.locator(".page-list-table")).to_contain_text("ListTest")


class TestPageViewing:
    def test_renders_markdown(self, page: Page, base_url: str, create_page):
        create_page("MarkdownPage", "# Title\n\n**bold** and *italic*")
        page.goto(f"{base_url}/page/MarkdownPage")
...
class TestPageEditing:
    def test_edit_existing_page(self, page: Page, base_url: str, create_page):
        create_page("EditMe", "# Original content")
        page.goto(f"{base_url}/page/EditMe")
        ...
        page.locator("button[type='submit']").click()
        page.wait_for_url(f"{base_url}/page/EditMe*")
        expect(page.locator(".page-content")).to_contain_text("Updated content")

    def test_edit_preserves_frontmatter(self, page: Page, base_url: str, data_dir):
        (data_dir / "FrontTest.md").write_text(
            "---\nstatus: draft\ntags:\n  - test\n---\n\n# Content",
            encoding="utf-8",
        )
        page.goto(f"{base_url}/page/FrontTest/edit")
...
class TestPageDeletion:
    def test_delete_page(self, page: Page, base_url: str, create_page):
        create_page("DeleteMe", "# To be deleted")
        page.goto(f"{base_url}/page/DeleteMe")
        page.on("dialog", lambda d: d.accept())
        page.locator("button.btn-danger").click()
        page.wait_for_url(f"{base_url}/*")
        page.goto(f"{base_url}/page/DeleteMe")
        expect(page).to_have_url(f"{base_url}/page/DeleteMe/edit")

    def test_cancel_delete(self, page: Page, base_url: str, create_page):
        create_page("KeepMe", "# Keep this")
        page.goto(f"{base_url}/page/KeepMe")
        page.on("dialog", lambda d: d.dismiss())
        page.locator("button.btn-danger").click()
        expect(page.locator(".page-header h1")).to_contain_text("KeepMe")
...
class TestPageList:
    ...
    def test_page_list_table(self, page: Page, base_url: str, create_page):
        create_page("Alpha", "# Alpha page")
        create_page("Beta", "# Beta page")
        page.goto(base_url)
